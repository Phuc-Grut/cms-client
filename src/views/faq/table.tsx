...
const Table = () => {
  ...
  const { checkInit, getFaqPagingApi, getFaqByIdApi, deleteFaqApi } = useFaq()
  ...
  useEffect(() => {
    if (
      ...
      getFaqByIdApi(doubleClickData.rowData.id)
        .unwrap()
        .then((rs: any) => {
          setDataItem(rs)
          handleModalDetail()
  ...
  useEffect(() => {
    if (!isObjEmpty(contextMenuClick)) {
      if (contextMenuClick.item.id === "EDIT") {
        getFaqByIdApi(contextMenuClick.rowInfo.rowData.id)
          .unwrap()
          .then((rs) => {
            setTimeout(() => {
              setDataItem(rs)
