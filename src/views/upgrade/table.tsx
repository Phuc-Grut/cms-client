...
import { UpgradeContext } from "./useContext"
...
const Table = () => {
  ...
  useEffect(() => {
    if (
      ...
      getUpgradeByIdApi(doubleClickData.rowData.id)
        .unwrap()
        .then((rs: any) => {
          setDataItem(rs)
          handleModalDetail()
  ...
  useEffect(() => {
    if (!isObjEmpty(contextMenuClick)) {
      if (contextMenuClick.item.id === "EDIT") {
        getUpgradeByIdApi(contextMenuClick.rowInfo.rowData.id)
          .unwrap()
          .then((rs) => {
            setTimeout(() => {
              setDataItem(rs)
