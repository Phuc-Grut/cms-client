...
const Table = () => {
  ...
  const { checkInit, getNewsPagingApi, getNewsByIdApi, deleteNewsApi } =
    useNews()
  ...
  useEffect(() => {
    if (
      ...
      getNewsByIdApi(doubleClickData.rowData.id)
        .unwrap()
        .then((rs: any) => {
          setDataItem(rs)
          handleModalDetail()
  ...
  useEffect(() => {
    if (!isObjEmpty(contextMenuClick)) {
      if (contextMenuClick.item.id === "EDIT") {
        getNewsByIdApi(contextMenuClick.rowInfo.rowData.id)
          .unwrap()
          .then((rs) => {
            setTimeout(() => {
              setDataItem(rs)
