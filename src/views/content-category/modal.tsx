...
import { IFDataContentCategory } from '@src/domain/models/IContentCategory'
...
const ModalComponent = () => {
  ...
  const { setParentCategory, openModal, handleModal, setDataItem, dataItem, typeModal, groupAssignedTo, parentCategory, breadCrumb } = useContext(ContentCategoryContext)
  const getWindowSize = () => {
  ...
  const defaultValues: IFDataContentCategory = {
    id: "",
    code: "",
    name: "",
    description: "",
  ...
  const onSubmit = (data: any) => {
    ...
    if (typeModal === 'Add') {
      addContentCategoryApi(obj).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
      ...
      editContentCategoryApi(obj).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title="Content" />
        <Scrollbars autoHide
          autoHeight
