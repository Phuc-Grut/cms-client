...
import { useGroupWebLink } from "../group-weblink/hooks"
...
import {canContextMenuItems} from "@src/views/weblink/contextMenu"
  ...
  return (
    <div className="d-flex justify-content-between align-items-center w-100">
      ...
      <div className="bar__action-right d-flex">
        <CanPer I={userAction.ADD} a={userSubject.WebLinkList}>
...
const Table = () => {
  ...
  const { getListGroupWebLinkApi } = useGroupWebLink()
  const handleModalSort = () => {
    getListWebLinkApi({
      $keyword: searchTerm,
      $status: statusAssignedTo,
      $groupWebLinkId: groupAssignedTo?.value ? groupAssignedTo?.value : undefined,
      $parentWebLinkId: location.state?.id ? location.state?.id : "null"
    })
      .unwrap()
      .then((rs: any) => {
        setDataSort(rs)
    ...
    getListGroupWebLinkApi({})
      ...
      .then(async (rs) => {
        ...
        if (rs && rs.length > 0) {
          ...
          const groupWebLinkLocal = await localStorage.getItem("groupWebLink")
          if (groupWebLinkLocal && groupWebLinkLocal !== "undefined") {
            setGroupAssignedTo(JSON.parse(groupWebLinkLocal))
          } else {
            setGroupAssignedTo(obj)
            localStorage.setItem("groupWebLink", JSON.stringify(obj))
          }
        }
      })
  ...
  const handleGroupAssignedToChange = (val: any) => {
    ...
    localStorage.setItem("groupWebLink", JSON.stringify(val))
    if (location?.state?.id) {
