...
import { IContext, GroupWebLinkContext } from './useContext'
import { useGroupWebLink } from './hooks'
...
const ModalOrderComponent = () => {
  ...
  const {typeModal, handleModalOrder, openModalOrder, windowSize, optionGroupWebLink} = useContext<IContext>(GroupWebLinkContext)
  ...
  const rowDataBound = (e: any) => {
    const lastBtn = e.data === optionGroupWebLink[optionGroupWebLink.length - 1] ? "lastBtn" : ""
    const firstBtn = e.data === optionGroupWebLink[0] ? "firstBtn" : ""
  ...
  const commandClick = (args: any) => {
    const index = optionGroupWebLink.findIndex((x: any) => x.id === args.rowData.id)
    if (gird?.current) {
      if (args.commandColumn.id === 'MOVEUP' && index !== 0) {
        gird.current.reorderRows([index], index - 1)
        rowDrop()
      } else if (args.commandColumn.id === 'MOVEDOWN' && index !== optionGroupWebLink.length - 1) {
        gird.current.reorderRows([index], index + 1)
        rowDrop()
      }
          ...
          <ModalBody>
            <div className='control-pane'>
              <div className='control-section'>
                ...
                >
                  <ColumnsDirective>
                    <ColumnDirective field='code' headerText={t("Code")} width='100'/>
                    <ColumnDirective field='name' headerText={t("Name")} width='150'/>
