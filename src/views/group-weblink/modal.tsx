...
import { IFDataGroupWebLink } from '@src/domain/models/IGroupWebLink'
...
const ModalComponent = () => {
  ...
  const { openModal, handleModal, dataItem, typeModal } = useContext(GroupWebLinkContext)
  // ** States
  const getWindowSize = () => {
  ...
  const formSchema = yup.object().shape({
    ...
    description: yup
      .string()
      .max(1000, (t("Description") + t(" must less than 500 characters")))
  })

  const defaultValues: IFDataGroupWebLink = {
    id: "",
    name: "",
    code: "",
    title:"",
  ...
  const onSubmit = (data: any) => {
    if (typeModal === 'Add') {
      addGroupWebLinkApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
      ...
      editGroupWebLinkApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title='Group WebLink' />
        <Scrollbars autoHide
          autoHeight
