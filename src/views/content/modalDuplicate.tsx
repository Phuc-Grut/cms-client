...
import { IFDuplicateContentApi } from '@src/domain/models/IContent'
...
const ModalDuplicate = () => {
  ...
  const { duplicateContentApi, getContentByIdApi, checkInit } = useContent()
  const { openModalDuplicate, handleModalDuplicate, dataItem, typeModal, setDataItem, setTypeModal, handleModal } = useContext(ContentContext)
  ...
  const defaultValues: IFDuplicateContentApi = {
    status: 1,
    isAuto: 1,
    ...
    return (
      <Fragment>
        <Button disabled={checkInit} color="primary" className="mb-75 me-1"  >
          <div onClick={() => { setTypeSubmit(0) }}>{t('Copy')}</div>
        </Button>
        <Button disabled={checkInit} color='primary' className='mb-75 me-1'>
          <div onClick={() => { setTypeSubmit(1) }}>{t('Copy and edit')}</div>
        </Button>
  ...
  const onSubmit = (data: any) => {
    ...
    if (typeModal === '') {
      if (typeSubmit === 0) {
        duplicateContentApi(obj).unwrap().then((rs) => {
          if (rs.isValid === true) {
            handleCancel()
        ...
        duplicateContentApi(obj).unwrap().then((rsA) => {
          if (rsA.isValid === true) {
            notificationSuccess(t('Copy Successfull'))
            handleCancel()
            getContentByIdApi(rsA.id)
              .unwrap()
              .then((rs) => {
                setTimeout(() => {
  ...
  return (
    ...
    >
      <Form id='form-modal-globalzone' 
        ...
        <ModalHeader handleModal={handleModalDuplicate} typeModal={typeModal} title='Content Duplicate' />
        <Scrollbars autoHide
          autoHeight
