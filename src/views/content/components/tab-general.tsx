...
import { useContentType } from "@src/views/content-type/hooks"
...
const TabGeneralInfo = (props: any) => {
  ...
  const { typeModal, openModal, dataItem, setOptionGrCategory, optionGrCategory, setValueTag, valueTag
  ...
  useEffect(() => {
    if (openModal === true) {
      if (optionContentType?.length === 0) {
        getListContentTypeApi({ $status: 1 }).unwrap()
          .then((rs) => {
            setTimeout(() => {
              setOptionContentType(rs)
              if (typeModal === 'Add') {
                setValue('contentTypeId', rs[0].value)
                setValue('contentType', rs[0].label)
              }
            }, 10)
          })
          .catch((ex) => {
      ...
      if (optionContentCategory?.length === 0) {
        getListComboBoxContentCategoryApi().unwrap()
          .then((rs) => {
            setTimeout(() => {
            ...
            <Row className='gy-1 mb-1'>
              <Col xs={12}>
                <SelectBox
                  ...
                  callback={(val: any) => {
                    if (val) {
                      setValue('contentType', val?.label)
                    } else {
