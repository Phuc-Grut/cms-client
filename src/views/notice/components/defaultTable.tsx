...
const DefaultTable = memo(
  ({
    ...
    const editOptions: EditSettingsModel =
      inlineEdit === true || inlineEdit === undefined ? {
        allowEditing: true,
        allowAdding: allowAdding === true || allowAdding === undefined,
        allowDeleting: allowDelete === true || allowDelete === undefined,
        mode: "Normal",
        ...
        if (args.requestType === "delete") {
          if (args.data[0].id !== null && args.data[0].id !== undefined) {
            ...
            const data = dataTable.filter(
              (x: any) => x.orderKey !== args.data[0].orderKey
            )
            setDataTable(data)
          }
        }
    ...
    useEffect(() => {
      if (dataGrid) {
        if (listContextShow?.type === "SHOW") {
          dataGrid.contextMenuModule.contextMenu.showItems(
            listContextShow.list,
            true
          )
        }
        if (listContextHide?.type === "HIDE") {
          dataGrid.contextMenuModule.contextMenu.hideItems(
            listContextHide.list,
            true
          )
        }
      }
    ...
    const rowSelecting = () => {
      if (
        ...
        dataGrid.current?.getSelectedRecords().length
      ) {
    ...
    return (
      <Fragment>
        <GridComponent
          ...
          selectionSettings={
            selectionSettings ? selectionSettings : groupSettingsDefault
          }
          rowSelected={rowSelected}
          rowDeselected={rowDeselected}
          rowSelecting={rowSelecting}
          recordDoubleClick={
            recordDoubleClick ? recordDoubleClickEvent : undefined
          }
          filterSettings={filterOptions}
          contextMenuOpen={contextMenuOpen}
          contextMenuItems={
            showContextMenu ? contextMenuItems?.map((x: any) => ({ ...x, text: t(x.text) })) : undefined
          }
