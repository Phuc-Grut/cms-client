...
const Table = () => {
  ...
  const { checkInit, getNoticePagingApi, getNoticeByIdApi, deleteNoticeApi } =
    useNotice()
  ...
  useEffect(() => {
    ...
    const rs = queryParam
      .map(
        (x: any) => `${x.key}${x.ope}${
          listDate.find((d) => d === x.key) ? moment(x.value).format("YYYY/MM/DD") : x.value
        }`
      )
      .join(";")
    getNoticePagingApi({
  ...
  useEffect(() => {
    if (
      ...
      getNoticeByIdApi(doubleClickData.rowData.id)
        .unwrap()
        .then((rs: any) => {
          setDataItem(rs)
          handleModalDetail()
  ...
  useEffect(() => {
    if (!isObjEmpty(contextMenuClick)) {
      if (contextMenuClick.item.id === "EDIT") {
        getNoticeByIdApi(contextMenuClick.rowInfo.rowData.id)
          .unwrap()
          .then((rs) => {
            setTimeout(() => {
              setDataItem(rs)
