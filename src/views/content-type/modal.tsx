...
import { IFDataContentType } from '@src/domain/models/IContentType'
...
const ModalComponent = () => {
  ...
  const { openModal, handleModal, dataItem, typeModal } = useContext(ContentTypeContext)
  // ** States
  const getWindowSize = () => {
  ...
  const formSchema = yup.object().shape({
    code: yup
      .string()
      .required((t("Content type code") + t(" is required")))
      .max(50, (t("Content type code") + t(" must less than 255 characters")))
      .min(2, (t("Content type code") + t(" must greater than 2 characters"))),
    name: yup.string()
      .required((t("Content type name") + t(" is required")))
      .max(255, t("Content type name") + t(" must less than 255 characters"))
      .min(2, (t("Content type name") + t(" must greater than 2 characters"))),
    description: yup
      .string()
      .max(500, (t("Description") + t(" must less than 500 characters")))
  })

  const defaultValues: IFDataContentType = {
    description: "",
    id: "",
    code: "",
    name: "",
  ...
  const renderFooterButtons = () => {
    if (typeModal === 'Edit' || typeModal === 'Add') {
      ...
      return (
        <Fragment>
          <Button disabled={checkInit} color="primary" className="mb-75 me-1" onClick={handleModal}>
            {t('Delete')}
          </Button>
          <Button color="secondary" onClick={handleCancel} outline className="mb-75 me-1">
            {t('Close')}
  ...
  const onSubmit = (data: any) => {
    if (typeModal === 'Add') {
      ...
      editContentTypeApi(data).unwrap()
        .then((rs) => {
          if (rs.isValid === true) {
            handleModal()
  ...
  return (
    ...
    >
      <Form id='form-modal-contentType' className='todo-modal' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader handleModal={handleModal} typeModal={typeModal} title='Content Type' />
        <Scrollbars autoHide
          autoHeight
