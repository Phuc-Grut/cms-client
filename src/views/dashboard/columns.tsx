...
const imageTemplate = (props: any) => {
  return (
    <img width={'35px'} src={(!isNullOrUndefined(props.img) && props.img !== '') ? (`${BASE_URL_CDN}/${props.img}`) : require('@src/assets/images/avatars/default-pro.jpg').default} />
  )
}
...
const nameTemplate = (props: any) => {
  return (
    <div>
      ...
      <p className='m-0' style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{props.name}</p>
    </div>
  )
}

export const columnsNewsContent = [
  {
    isPrimaryKey: true,
    field: "image",
    headerText: "Image",
