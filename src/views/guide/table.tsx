...
const Table = () => {
  ...
  const { checkInit, getGuidePagingApi, getGuideByIdApi, deleteGuideApi } =
    useGuide()
  ...
  useEffect(() => {
    if (
      ...
      getGuideByIdApi(doubleClickData.rowData.id)
        .unwrap()
        .then((rs: any) => {
          setDataItem(rs)
          handleModalDetail()
  ...
  useEffect(() => {
    if (!isObjEmpty(contextMenuClick)) {
      if (contextMenuClick.item.id === "EDIT") {
        getGuideByIdApi(contextMenuClick.rowInfo.rowData.id)
          .unwrap()
          .then((rs) => {
            setTimeout(() => {
              setDataItem(rs)
